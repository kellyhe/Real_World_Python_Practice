...
def FreqDist_freg(work_list):
    """Define a new version of the nltk.FreqDist() method that uses 
    percentages, rather than counts."""
    fq = nltk.FreqDist(work_list)
    norm = fq.N()
    for f in fq:
        fq[f] = fq[f]/norm
    return fq
...
def word_length_test(words_by_author, len_shortest_corpus):
    ...
    for i, author in enumerate(words_by_author):
        word_lengths = [len(word) for word in words_by_author[author]
                        [:len_shortest_corpus]]
        by_author_length_freq_dist[author] = FreqDist_freg(word_lengths)
        by_author_length_freq_dist[author].plot(15,
                                                linestyle=LINES[i],
                                                label=author,
                                                title='Word Length',cumulative=True)
...
def stopwords_test(words_by_author, len_shortest_corpus):
    ...
    for i, author in enumerate(words_by_author):
        ...
        stopwords_by_author_freq_dist[author] = FreqDist_freg(stopwords_by_author)    
...
def parts_of_speech_test(words_by_author, len_shortest_corpus):
    ...
    for i, author in enumerate(words_by_author):
        pos_by_author = [pos[1] for pos in nltk.pos_tag(words_by_author[author]
                                                        [:len_shortest_corpus])] 
        by_author_pos_freq_dist[author] = FreqDist_freg(pos_by_author)
    ...
    for author in words_by_author:
        if author != 'unknown': 
            ...
            author_proportion = (len(words_by_author[author])/
                                 len(combined_corpus))
            combined_freq_dist = FreqDist_freg(combined_corpus)
            most_common_words = list(combined_freq_dist.most_common(1000))
            chisquared = 0

            # Calculate observed (sample) vs. expected word counts (population).
