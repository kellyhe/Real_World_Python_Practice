...
#------------------------------------------------------------------------------
# Challenge: concatenate a strip along the eastern margin wide enough 
# to accommodate the final overlapping rectangle.

# Opens a image in RGB mode
IMG_GRAY_PIL = Image.open("mola_1024x501.png")
# CROP ORIGINAL IMAGE
IMG_GRAY_PIL_CROP = IMG_GRAY_PIL.crop((0, 0, RECT_WIDTH, IMG_HT)) #left, top, right, bottom
#convert to openCV format
IMG_GRAY_CROP = cv.cvtColor(np.array(IMG_GRAY_PIL_CROP), cv.COLOR_BGR2GRAY)
# horizontally concatenates images of same height 
IMG_GRAY_UPDATE = cv.hconcat([IMG_GRAY, IMG_GRAY_CROP])
# show the UPDATED image
cv.imshow('Updated mola', IMG_GRAY_UPDATE)
cv.waitKey(3000)

IMG_HT, IMG_WIDTH_UPDATE = IMG_GRAY_UPDATE.shape
...
class Search():
    ...
    def run_rect_stats(self):
        ...
        while True:
            rect_img = IMG_GRAY_UPDATE[ul_y : lr_y, ul_x : lr_x]
    ...
    def draw_filtered_rects(self, image, filtered_rect_list):
        ...
        cv.line(img_copy, (0, LAT_30_N), (IMG_WIDTH_UPDATE, LAT_30_N),
                (255, 0, 0), 1)
        cv.line(img_copy, (0, LAT_30_S), (IMG_WIDTH_UPDATE, LAT_30_S),
                (255, 0, 0), 1)
...
def main():
    ...
    app.draw_qc_rects()
    app.sort_stats()
    ptp_img = app.draw_filtered_rects(IMG_GRAY_UPDATE, app.ptp_filtered)
    std_img = app.draw_filtered_rects(IMG_GRAY_UPDATE, app.std_filtered)
